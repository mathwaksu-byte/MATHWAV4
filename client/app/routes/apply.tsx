...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  const uploadMarksheet = async () => {
    ...
    if (apiBinding?.fetch) {
      try {
        ...
        const r = await apiBinding.fetch(new Request('https://api.local/api/uploads/single', { method: 'POST', body: upForm }));
  ...
  try {
    ...
    if (apiBinding?.fetch) {
      try {
        const r = await apiBinding.fetch(new Request('https://api.local/api/applications', {
          method: 'POST',
